import { useState, useEffect, useMemo, ReactElement, isValidElement, cloneElement, Children, createContext, useContext } from "react";
import { useNavigate, useLocation } from "react-router";
import {
  LayoutGrid, Building2, FileText, Users, Wallet, Settings, LifeBuoy, LogOut,
  Bell, Home, MessageSquare, User, ChevronRight, X, Search, Lock, Check, CreditCard,
} from "lucide-react";
...
function Sidebar({ active, subStatus, handleToggleSub, onSaaSAccess }: { 
  active: LandlordNavId; subStatus: string; handleToggleSub: (s: any) => void; onSaaSAccess: () => void 
}) {
  const navigate = useNavigate();
  ...
  return (
    <aside style={{ width: 248, background: C.white, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", flexShrink: 0 }}>
      ...
      <div style={{ padding: "10px 12px 14px", borderTop: `1px solid ${C.border}` }}>
        ...
        <select 
          value={subStatus}
          onChange={e => handleToggleSub(e.target.value as any)}
          style={{ 
...
function TrialRegisterModal({ open, onCancel, onConfirm }: { open: boolean; onCancel: () => void; onConfirm: () => void }) {
  if (!open) return null;
  return (
    <>
...
export function LandlordShell({ active, mobileTitle, children }: { active: LandlordNavId; mobileTitle: string; children: React.ReactNode }) {
  ...
  const updateSubStatus = (status: "NONE" | "TRIAL" | "ACTIVE" | "READ_ONLY") => {
    setSubStatus(status);
    window.dispatchEvent(new CustomEvent("tronhanh_sub_status", { detail: status }));
  };
  ...
  const handleToggleSub = async (newStatus: "NONE" | "TRIAL" | "ACTIVE" | "READ_ONLY") => {
    if (!user) return;
    try {
      setLoadingSub(true);
      ...
      const planDuration = newStatus === "TRIAL" ? 30 : 365;
      const expireDate = new Date(Date.now() + planDuration * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  const renderBanner = () => {
    if (subStatus === "TRIAL") {
      return (
        <div style={{ background: "#FEF6EC", borderBottom: `1px solid ${C.border}`, padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <button 
            ...
            style={{ padding: "6px 14px", background: C.primary, color: C.white, border: "none", borderRadius: 8, fontFamily: font, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
            Nâng cấp gói
          </button>
        </div>
      );
    }
    if (subStatus === "READ_ONLY") {
      return (
        <div style={{ background: "#FCECEC", borderBottom: `1px solid #FFEBEB`, padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <button 
            onClick={() => setShowPaymentModal(true)}
            style={{ padding: "6px 14px", background: C.repairing, color: C.white, border: "none", borderRadius: 8, fontFamily: font, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
            Gia hạn gói
          </button>
        </div>
      );
  ...
  const activeTab = useMemo(() => {
    try {
      // React Router's location.search handles query params reactively
  ...
  const renderContent = () => {
    if (isSaaSBlocked) {
      return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 160px)", padding: 32, textAlign: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: C.caramelSoft, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
            <Lock size={28} color={C.primary} />
          </div>
          <h2 style={{ fontFamily: font, fontSize: 22, fontWeight: 800, color: C.textPrimary, margin: "0 0 8px" }}>
            Tính năng dành riêng cho gói SaaS
          </h2>
          <p style={{ fontFamily: font, fontSize: 14, color: C.textSecondary, margin: "0 0 24px", maxWidth: 440, lineHeight: 1.6 }}>
            Quản lý khu trọ, ghi nhận chỉ số điện nước, và tự động tạo hóa đơn thanh toán qua VietQR. Đăng ký dùng thử 30 ngày hoàn toàn miễn phí ngay!
          </p>
          <button
            onClick={handleSaaSAccess}
            style={{ padding: "12px 24px", background: C.primary, color: C.white, border: "none", borderRadius: 12, fontFamily: font, fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px rgba(138,106,69,0.3)" }}
          >
    ...
    return (
      <LandlordShellContext.Provider value={{ subStatus, activeTab }}>
        <div style={{ background: C.bg, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
          ...
          <MobileTabBar active={activeTab} subStatus={subStatus} onSaaSAccess={handleSaaSAccess} />
          <TrialRegisterModal open={showTrialRegister} onCancel={() => setShowTrialRegister(false)} onConfirm={() => { handleToggleSub("TRIAL"); setShowTrialRegister(false); }} />
          <SaaSPaymentModal open={showPaymentModal} onClose={() => setShowPaymentModal(false)} onConfirm={() => handleToggleSub("ACTIVE")} />
        </div>
  ...
  return (
    <LandlordShellContext.Provider value={{ subStatus, activeTab }}>
      <div style={{ background: C.bg, minHeight: "100vh", display: "flex" }}>
        <Sidebar active={activeTab} subStatus={subStatus} handleToggleSub={handleToggleSub} onSaaSAccess={handleSaaSAccess} />
        ...
        <div style={{ padding: "24px", overflowY: "auto", flex: 1, textAlign: "left" }} className="tn-scroll-y">
          ...
          {step === "SUCCESS" && (
            <div style={{ textAlign: "center", padding: "16px 0" }}>
              ...
              <p style={{ fontFamily: font, fontSize: 13.5, color: C.textSecondary, margin: "0 0 16px", lineHeight: 1.6 }}>
                Giao dịch của bạn đã được đối soát thành công. Tài khoản của bạn đã được chuyển đổi sang trạng thái <strong style={{ color: "#4A7A34" }}>Kích hoạt (ACTIVE)</strong> và có thời hạn sử dụng 1 năm kể từ hôm nay.
              </p>
