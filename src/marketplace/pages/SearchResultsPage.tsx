...
function applyFilters(rooms: Room[], f: SearchFilters): Room[] {
  return rooms.filter(r => {
    ...
    if (f.type && f.type !== "Tất cả") {
      ...
      if (f.type === "Căn hộ mini" && r.type !== "mini") return false;
      if (f.type === "Căn hộ dịch vụ" && r.type !== "apartment") return false;
      if (f.type === "Ký túc xá" && r.type !== "ktx") return false;
      if (f.type === "Nhà nguyên căn" && r.type !== "house") return false;
    }
...
export function SearchResultsPage() {
  ...
  const filteredRooms = applyFilters(dbRooms, filters);
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {viewMode === "map" ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "16px 16px 24px" }}>
              <div style={{ fontFamily: font, fontSize: 14, color: C.textSecondary, textAlign: "center", padding: 32 }}>Đang tìm kiếm phòng phù hợp...</div>
            </div>
          ) : filteredRooms.length > 0 ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "16px 16px 24px" }}>
              {filteredRooms.map(r => <RoomCard key={r.id} room={r} mobile onClick={() => navigate(`/phong/${r.id}`)} />)}
            </div>
          ) : (
      ...
      <div style={{ flex: 1, display: "flex", maxWidth: 1280, margin: "0 auto", width: "100%", padding: "0 32px", gap: 28, alignItems: "flex-start" }}>
        ...
        <main style={{ flex: 1, minWidth: 0, paddingTop: 28, paddingBottom: 72 }}>
          ...
          {viewMode === "map" ? (
            ...
            <div style={{ fontFamily: font, fontSize: 14, color: C.textSecondary, textAlign: "center", padding: 72 }}>Đang tìm kiếm phòng phù hợp...</div>
          ) : filteredRooms.length > 0 ? (
            <>
              <div style={{ display: "grid", gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gap: 20 }}>
                {filteredRooms.map(r => <RoomCard key={r.id} room={r} onClick={() => navigate(`/phong/${r.id}`)} />)}
              </div>
              <div style={{ textAlign: "center", marginTop: 48 }}>
                <Btn variant="outline" label="Quay lại" size="lg" onClick={onBack} />
              </div>
            </>
          ) : (
